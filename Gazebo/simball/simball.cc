    ...
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
            {
               ...
               this->updateConnection = event::Events::ConnectWorldUpdateBegin(
                  ...
                  this->pub_pos = this->rosNode->advertise<geometry_msgs::Pose>("/ball_position", 30);
                  //this->pub_moving = this->rosNode->advertise<std_msgs::Bool>("/cylinder_motion", 30);
                  this->pub_object_sight = this->rosNode->advertise<std_msgs::Float64>("/ball_visible", 30);
                  ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>("/simball/commands_object",1,boost::bind(&BallPlugin::OnRosMsg, this, _1),ros::VoidPtr(), &this->rosQueue);
                  this->rosSub = this->rosNode->subscribe(so);
                  this->rosQueueThread = std::thread(std::bind(&BallPlugin::QueueThread, this));
                  ros::SubscribeOptions sub_o = ros::SubscribeOptions::create<std_msgs::Bool>("/motion_panda/end_action",1,boost::bind(&BallPlugin::OnRosMsgAction, this, _1),ros::VoidPtr(), &this->rosQueueAction);
                  this->endAction = this->rosNode->subscribe(sub_o);
                  this->rosQueueThreadAction = std::thread(std::bind(&BallPlugin::QueueThreadAction, this));
                  activate_placement = true;
            }

    ...
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
            {
               ...
               this->updateConnection = event::Events::ConnectWorldUpdateBegin(
                  ...
                  this->pub_pos = this->rosNode->advertise<geometry_msgs::Pose>("/cube_position", 30);
                  //this->pub_moving = this->rosNode->advertise<std_msgs::Bool>("/cylinder_motion", 30);
                  this->pub_object_sight = this->rosNode->advertise<std_msgs::Float64>("/cube_visible", 30);
                  ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float32>("/simcube/commands_object",1,boost::bind(&CubePlugin::OnRosMsg, this, _1),ros::VoidPtr(), &this->rosQueue);
                  this->rosSub = this->rosNode->subscribe(so);
                  this->rosQueueThread = std::thread(std::bind(&CubePlugin::QueueThread, this));
                  ros::SubscribeOptions sub_o = ros::SubscribeOptions::create<std_msgs::Bool>("/motion_panda/end_action",1,boost::bind(&CubePlugin::OnRosMsgAction, this, _1),ros::VoidPtr(), &this->rosQueueAction);
                  this->endAction = this->rosNode->subscribe(sub_o);
                  this->rosQueueThreadAction = std::thread(std::bind(&CubePlugin::QueueThreadAction, this));
                  activate_placement = false;
               ...
               if((x < 0.25) || (x > 0.6) || (y < -0.25) || (y > 0.25))
                {
                        if(end_action == true && activate_placement == true)
                        {
                                ...
                                pos = ignition::math::Vector3d(0.40,0.0,1.1);
